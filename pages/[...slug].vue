  ...
  <div>
    <template v-if="page">
      <article class="article">
        ...
        <section class="prose"><ContentRenderer :value="page" /></section>

        <footer class="article-footer">
          <NuxtLink to="/" class="btn-pill">返回首页</NuxtLink>
        </footer>
      </article>
    </template>
    <div v-else-if="!pending" class="empty">
      <h2>404</h2><p>找不到这篇文章</p>
      <NuxtLink to="/" class="btn-pill">返回首页</NuxtLink>
    </div>
  </div>
</template>
...
if (page.value && (page.value.path === '/' || page.value.path === '/about')) page.value = null
</script>
